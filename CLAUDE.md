...
The `hydrogen` page's periodic table (`components/PeriodicTable.tsx`, Z=1-36 selectable) drives every visualization on the page via a `selectedZ`/`Z` prop threaded down from `page.tsx`; it's collapsed by default behind a `.elementBadge` button in the top nav (`page.tsx`'s `energyNav`) and auto-collapses again on any selection (`periodicTableCollapse` in `styles/HydrogenAtom.css`). The Spectrum tab (`components/HydrogenSpectrum.tsx`) branches on `Z`: hydrogen (Z=1) draws its lines from the exact Rydberg formula (`buildLines`, hydrogen's real named series); every other element instead uses real measured lines (`buildRealLines`, grouped/colored by ionization stage) from `data/nistSpectralLines.ts`, sourced from NIST's Handbook of Basic Atomic Spectroscopic Data "Persistent Lines" tables — see that file's header comment for the sourcing/units/caveats. That data file is generated, not hand-written: `nist/nistScraper.py` fetches and saves each element's raw source pages under `nist/<Element>/` (same saved-source-for-provenance convention as `solarsystem/wikipedia/`), and `nist/generateTs.py` converts the parsed JSON straight into the `.ts` data file — regenerate both rather than hand-editing entries if the source data changes. Global CSS gotcha worth knowing before adding new `<span>`s anywhere on the site: `globals.css` sets `p, li, span { color: var(--text-primary) }` (a dark green), which wins over any color set on an ancestor element since inheritance has no specificity — give new spans an explicit `color` rather than relying on inheriting the parent's.
